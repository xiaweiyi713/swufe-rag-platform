...
## 2026-07-21：证据先行、声明级验证真流式

### 实现

- 政策 RAG 在检索证据冻结后消费模型真实 token 流，完整句/分句进入隔离缓冲区；只有引用位置、来源相关性、数字/课程代码和 URL 检查全部通过后才提交 `verified delta`。
- 流结束后再次执行完整答案校验。任一声明或终检失败时关闭上游流，发送 `reset` 并替换为确定性证据答案；被拒绝的模型文本不会进入客户端事件。
- 新增有界队列和断线取消信号。HTTP 消费端关闭后，提供商生成器会被关闭，工作线程退出后才释放会话保护。
- iOS 保留原 `StreamRevealBuffer` 逐字节奏，只新增 `reset` 处理以原子清空未展示字符并替换安全回答。
- 已识别政策主题改为可信 SQLite 权威文档直取；只有没有精确选择器时才进入向量检索与重排。真实“推免基本条件”场景由约 30.8 秒降至约 2.8 秒。
- 百炼业务空间专属 `llm-*.cn-*.maas.aliyuncs.com` 域名采用受约束规则放行，仍要求 HTTPS/443 并拒绝私网、回环和云元数据地址。

### 验证

- 后端全量：`407 passed, 2 skipped, 69 warnings, 35 subtests passed`。
- iOS：iPhone 17 Pro / iOS 26.5 模拟器目标使用本机临时签名构建成功。
- 真实 Qwen `qwen3.5-plus` 场景：政策 RAG 2.8 秒、培养方案 SQL 1.8 秒、同会话范围追问 3.1 秒、普通聊天 1.0 秒、可信缓存命中 0.0 秒；回答、引用、来源下载入口均正常。
- Docker `swufe-rag-v16` 的 app 与 Redis 容器均通过健康检查，`/readyz` 返回 ready。
